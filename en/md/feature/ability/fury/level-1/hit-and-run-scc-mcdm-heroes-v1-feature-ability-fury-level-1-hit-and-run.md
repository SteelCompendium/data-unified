---
action_type: Main action
class: fury
distance: Melee 1
effect: You can shift 1 square.
flavor: Staying in constant motion helps you slip out of reach after a brutal assault.
keywords:
    - Melee
    - Strike
    - Weapon
level: "1"
name: '[Hit and Run](scc:mcdm.heroes.v1/feature.ability.fury.level-1/hit-and-run)'
power_roll_characteristic: Might
scc: mcdm.heroes.v1/feature.ability.fury.level-1/hit-and-run-scc-mcdm-heroes-v1-feature-ability-fury-level-1-hit-and-run
subtype: signature
target: One creature or object
tier1: 2 + M damage
tier2: 5 + M damage
tier3: 7 + M damage; A < STRONG, slowed (save ends)
type: ability
---


*Staying in constant motion helps you slip out of reach after a brutal assault.*

| **Melee, Strike, Weapon** |               **Main action** |
|---------------------------|------------------------------:|
| **📏 Melee 1**            | **🎯 One creature or object** |

**Power Roll + Might:**

- **≤11:** 2 + M damage
- **12-16:** 5 + M damage
- **17+:** 7 + M damage; A < STRONG, slowed (save ends)

**Effect:** You can shift 1 square.
