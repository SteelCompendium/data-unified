---
file_basename: star-hunter
file_dpath: treasure/leveled/armor
flavor: Shimmering light flows like liquid along this suit of crystalline armor.
item_id: star-hunter
item_name: Star-Hunter
item_prerequisite: A large vessel of astral ice, a pint of supercooled mercury
keywords:
    - Heavy Armor
    - Psionic
level_effects:
    1st: While you wear this armor, you gain a +6 [bonus](scc.v1:mcdm.heroes.v1/rule.dice/bonuses-and-penalties) to [Stamina](scc.v1:mcdm.heroes.v1/rule.health/stamina), but any magic ability gains an [edge](scc.v1:mcdm.heroes.v1/rule.dice/edge) when targeting you. Additionally, you instinctively know the location of any creature with [concealment](scc.v1:mcdm.heroes.v1/rule.combat/concealment) within 2 squares. You can also [turn](scc.v1:mcdm.heroes.v1/rule.combat/turn) invisible as a maneuver. Your invisibility ends if you take damage or use an ability, or at the end of your next [turn](scc.v1:mcdm.heroes.v1/rule.combat/turn).
    5th: The armor's [bonus](scc.v1:mcdm.heroes.v1/rule.dice/bonuses-and-penalties) to [Stamina](scc.v1:mcdm.heroes.v1/rule.health/stamina) increases to +12, you instinctively know the location of any creature with [concealment](scc.v1:mcdm.heroes.v1/rule.combat/concealment) within 5 squares, and your invisibility no longer ends at the end of your next [turn](scc.v1:mcdm.heroes.v1/rule.combat/turn). Additionally, you have psychic immunity 5.
    9th: The armor's [bonus](scc.v1:mcdm.heroes.v1/rule.dice/bonuses-and-penalties) to [Stamina](scc.v1:mcdm.heroes.v1/rule.health/stamina) increases to +21, and you instinctively know the location of any creature with [concealment](scc.v1:mcdm.heroes.v1/rule.combat/concealment) within 10 squares. Your invisibility no longer ends when you use an ability, and you have psychic immunity 10.
name: Star-Hunter
project_goal: "450"
project_roll_characteristic: '[Might](scc.v1:mcdm.heroes.v1/rule.character/might), [Reason](scc.v1:mcdm.heroes.v1/rule.character/reason), or [Intuition](scc.v1:mcdm.heroes.v1/rule.character/intuition)'
project_source: Texts or lore in Voll
scc: mcdm.heroes.v1/treasure.leveled.armor/star-hunter
source: mcdm.heroes.v1
treasure_type: armor
type: treasure
---

*Shimmering light flows like liquid along this suit of crystalline armor.*

**Keywords:** Heavy Armor, Psionic

**[Item Prerequisite](scc.v1:mcdm.heroes.v1/rule.downtime/item-prerequisite):** A large vessel of astral ice, a pint of supercooled mercury

**[Project Source](scc.v1:mcdm.heroes.v1/rule.downtime/project-source):** Texts or lore in Voll

**[Project Roll](scc.v1:mcdm.heroes.v1/rule.downtime/project-roll) [Characteristic](scc.v1:mcdm.heroes.v1/rule.character/characteristic):** [Might](scc.v1:mcdm.heroes.v1/rule.character/might), [Reason](scc.v1:mcdm.heroes.v1/rule.character/reason), or [Intuition](scc.v1:mcdm.heroes.v1/rule.character/intuition)

**Project Goal:** 450

**1st Level:** While you wear this armor, you gain a +6 [bonus](scc.v1:mcdm.heroes.v1/rule.dice/bonuses-and-penalties) to [Stamina](scc.v1:mcdm.heroes.v1/rule.health/stamina), but any magic ability gains an [edge](scc.v1:mcdm.heroes.v1/rule.dice/edge) when targeting you. Additionally, you instinctively know the location of any creature with [concealment](scc.v1:mcdm.heroes.v1/rule.combat/concealment) within 2 squares. You can also [turn](scc.v1:mcdm.heroes.v1/rule.combat/turn) invisible as a maneuver. Your invisibility ends if you take damage or use an ability, or at the end of your next [turn](scc.v1:mcdm.heroes.v1/rule.combat/turn).

**5th Level:** The armor's [bonus](scc.v1:mcdm.heroes.v1/rule.dice/bonuses-and-penalties) to [Stamina](scc.v1:mcdm.heroes.v1/rule.health/stamina) increases to +12, you instinctively know the location of any creature with [concealment](scc.v1:mcdm.heroes.v1/rule.combat/concealment) within 5 squares, and your invisibility no longer ends at the end of your next [turn](scc.v1:mcdm.heroes.v1/rule.combat/turn). Additionally, you have psychic immunity 5.

**9th Level:** The armor's [bonus](scc.v1:mcdm.heroes.v1/rule.dice/bonuses-and-penalties) to [Stamina](scc.v1:mcdm.heroes.v1/rule.health/stamina) increases to +21, and you instinctively know the location of any creature with [concealment](scc.v1:mcdm.heroes.v1/rule.combat/concealment) within 10 squares. Your invisibility no longer ends when you use an ability, and you have psychic immunity 10.
