---
agility: -1
ev: 10 for four minions
file_basename: frost-giant-snowblaster
file_dpath: monster/giant/statblock
free_strike: 4
immunities:
    - Cold 8
intuition: 4
item_id: frost-giant-snowblaster
item_name: Frost Giant Snowblaster
keywords:
    - Frost Giant
    - Giant
level: 8
might: 3
name: Frost Giant Snowblaster
organization: Minion
presence: 2
reason: -1
role: Artillery
scc: mcdm.monsters.v1/monster.giant.statblock/frost-giant-snowblaster
size: "4"
source: mcdm.monsters.v1
speed: 7
stability: 5
stamina: "12"
type: statblock
with_captain: +3 damage bonus to strikes
---

```ds-sb
agility: -1
ev: 10 for four minions
features:
    - ability_type: Signature Ability
      distance: Ranged 12
      effects:
        - roll: Power Roll + 4
          tier1: 4 cold damage
          tier2: 7 cold damage
          tier3: 9 cold damage; prone
      feature_type: ability
      icon: "\U0001F3F9"
      keywords:
        - Magic
        - Ranged
        - Strike
      name: Slushfall
      target: One creature or object per minion
      type: feature
      usage: Main action
    - effects:
        - effect: The snowblaster is surrounded by a snowstorm. Any enemy who starts their turn within 2 squares of the snowblaster can't [shift](../../../movement/shifting.md).
      feature_type: trait
      icon: ⭐️
      name: Kingdom of Isolation
      type: feature
free_strike: 4
immunities:
    - Cold 8
intuition: 4
keywords:
    - Frost Giant
    - Giant
level: 8
metadata:
    scc: mcdm.monsters.v1/monster.giant.statblock/frost-giant-snowblaster
    source: mcdm.monsters.v1
might: 3
name: Frost Giant Snowblaster
organization: Minion
presence: 2
reason: -1
role: Artillery
size: "4"
speed: 7
stability: 5
stamina: "12"
type: statblock
with_captain: +3 damage bonus to strikes
```
