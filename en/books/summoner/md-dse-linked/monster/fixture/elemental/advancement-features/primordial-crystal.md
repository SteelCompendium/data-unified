---
features:
    - body: Each [round](../../../../rule/combat/combat-round.md), you gain a [surge](../../../../rule/resource/surge.md) the first time an area of terrain gains a [supernatural](../../../../rule/general/supernatural.md) effect (excluding [auras](../../../../rule/combat/aura.md)) while you have [line of effect](../../../../rule/combat/line-of-effect.md) to the crystal. You can choose to give the [surge](../../../../rule/resource/surge.md) to an ally who also has [line of effect](../../../../rule/combat/line-of-effect.md) to the crystal.
      effects:
        - effect: Each [round](../../../../rule/combat/combat-round.md), you gain a [surge](../../../../rule/resource/surge.md) the first time an area of terrain gains a [supernatural](../../../../rule/general/supernatural.md) effect (excluding [auras](../../../../rule/combat/aura.md)) while you have [line of effect](../../../../rule/combat/line-of-effect.md) to the crystal. You can choose to give the [surge](../../../../rule/resource/surge.md) to an ally who also has [line of effect](../../../../rule/combat/line-of-effect.md) to the crystal.
      icon: ⭐️
      level: 5
      name: Terra Resonance
    - body: The crystal is now size 3.
      effects:
        - effect: The crystal is now size 3.
      icon: ⭐️
      level: 9
      name: Size Increase
    - body: When you or an ally makes a ranged [strike](../../../../rule/combat/strike.md) that draws a line through the crystal, the user gains a [surge](../../../../rule/resource/surge.md) which they can use on the ability.
      effects:
        - effect: When you or an ally makes a ranged [strike](../../../../rule/combat/strike.md) that draws a line through the crystal, the user gains a [surge](../../../../rule/resource/surge.md) which they can use on the ability.
      icon: ⭐️
      level: 9
      name: Magnified Strike
file_basename: primordial-crystal
file_dpath: monster/fixture/elemental/advancement-features
item_id: primordial-crystal
item_name: Primordial Crystal Advancement Features
name: Primordial Crystal Advancement Features
scc: mcdm.summoner.v1/monster.fixture.elemental.advancement-features/primordial-crystal
source: mcdm.summoner.v1
type: featureblock
---

```ds-fb
features:
    - body: Each [round](../../../../rule/combat/combat-round.md), you gain a [surge](../../../../rule/resource/surge.md) the first time an area of terrain gains a [supernatural](../../../../rule/general/supernatural.md) effect (excluding [auras](../../../../rule/combat/aura.md)) while you have [line of effect](../../../../rule/combat/line-of-effect.md) to the crystal. You can choose to give the [surge](../../../../rule/resource/surge.md) to an ally who also has [line of effect](../../../../rule/combat/line-of-effect.md) to the crystal.
      effects:
        - effect: Each [round](../../../../rule/combat/combat-round.md), you gain a [surge](../../../../rule/resource/surge.md) the first time an area of terrain gains a [supernatural](../../../../rule/general/supernatural.md) effect (excluding [auras](../../../../rule/combat/aura.md)) while you have [line of effect](../../../../rule/combat/line-of-effect.md) to the crystal. You can choose to give the [surge](../../../../rule/resource/surge.md) to an ally who also has [line of effect](../../../../rule/combat/line-of-effect.md) to the crystal.
      icon: ⭐️
      level: 5
      name: Terra Resonance
    - body: The crystal is now size 3.
      effects:
        - effect: The crystal is now size 3.
      icon: ⭐️
      level: 9
      name: Size Increase
    - body: When you or an ally makes a ranged [strike](../../../../rule/combat/strike.md) that draws a line through the crystal, the user gains a [surge](../../../../rule/resource/surge.md) which they can use on the ability.
      effects:
        - effect: When you or an ally makes a ranged [strike](../../../../rule/combat/strike.md) that draws a line through the crystal, the user gains a [surge](../../../../rule/resource/surge.md) which they can use on the ability.
      icon: ⭐️
      level: 9
      name: Magnified Strike
metadata:
    scc: mcdm.summoner.v1/monster.fixture.elemental.advancement-features/primordial-crystal
    source: mcdm.summoner.v1
name: Primordial Crystal Advancement Features
type: featureblock
```
