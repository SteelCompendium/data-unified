...
features:
    ...
    - body: A creature or object enters the corrosive pool or starts their turn there. The liquid in the pool is also highly volatile (see **Explosive Reaction** below).
      effects:
        - effect: A creature or object enters the corrosive pool or starts their turn there. The liquid in the pool is also highly volatile (see **Explosive Reaction** below).
    ...
    - distance: 3 burst
      effects:
        - effect: The pool takes fire damage.
          name: Trigger
          roll: Power Roll + 2
          tier1: 3 fire damage; M < 1 the target is burning ([save](scc.v1:mcdm.heroes.v1/rule.general/saving-throw) ends)
          tier2: 6 fire damage; M < 2 the target is burning ([save](scc.v1:mcdm.heroes.v1/rule.general/saving-throw) ends)
          tier3: 9 fire damage; M < 3 the target is burning ([save](scc.v1:mcdm.heroes.v1/rule.general/saving-throw) ends)
    ...
    - body: Allies who have weapons are equipped with torches. Any ally can use a maneuver to throw a torch up to 5 squares and deal 1 fire damage to the pool, triggering Explosive Reaction.
      effects:
        - effect: Allies who have weapons are equipped with torches. Any ally can use a maneuver to throw a torch up to 5 squares and deal 1 fire damage to the pool, triggering Explosive Reaction.
      icon: ⭐️
